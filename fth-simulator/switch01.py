...
class _Switch(object):
    ...
    def __init__(self, id, num_node=0, num_gpu_p_node=0, num_cpu_p_node=0, mem_p_node=0):
        ...
        self.id = id
        self.node_list = list()
        util01.print_fn('  Switch[%d] has %d nodes' % (id, num_node))
    ...
    def try_single_node_alloc(self, job):
        ...
        for node in self.node_list:
            if (node.check_free_gpus() >= need_gpu) and (node.check_free_cpus() >= need_cpu) and (node.free_mem >= JOBS01.worker_mem):
                ...
                traffic = JOBS01.create_single_node_placement(job, self.id, node.id, need_gpu, need_cpu, JOBS01.worker_mem)
                # node.add_network_load(traffic, traffic)
