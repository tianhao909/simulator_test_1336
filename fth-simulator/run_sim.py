...
def gandiva_sim_jobs(gputime=False, solve_starvation=0):
    ...
    '''
    cur_time = JOBS01.job_events[0]['time']
    node_release = False
    time_diff = 0
    while (len(JOBS01.job_events) + len(JOBS01.pending_jobs) + len(JOBS01.running_jobs))> 0:
        # if len(JOBS.job_events) == 0:
        #     break
        CLUSTER01.gandiva_node_set_adjust(cur_time, JOBS01, LOG01)
        print("%d-%d, %d, %d " % (cur_time, len(JOBS01.job_events), len(JOBS01.pending_jobs), len(JOBS01.running_jobs)))
        #update job progress for end_jobs
        node_release = CLUSTER01.time_slicing_execute(cur_time, JOBS01, LOG01, time_diff)

        #get new start job
        event = util01.search_dict_list(JOBS01.job_events, 'time', cur_time)
        if event != None:
            ...
            for s_job in event['start_jobs']:
                ...
                if ret == False:
                    ...
                    JOBS01.running_jobs.append(s_job)
                    util01.print_fn('----job[%d] starts' % s_job['job_idx'])

            #remove time_event
            JOBS01.job_events.remove(event)

        if node_release: 
            for p_job in JOBS01.pending_jobs:
                ret = try_get_job_res(p_job)
                if ret == True:
                    JOBS01.remove_from_pending(p_job, cur_time)
                    p_job['start_time'] = cur_time
                    JOBS01.running_jobs.append(p_job)
                    util01.print_fn('----job[%d] starts from pending' % p_job['job_idx'])
        ...
        if len(JOBS01.job_events) <= 0:
            ...
            next_e_time = JOBS01.job_events[0]['time']
...
def main():
    ...
    if FLAGS01.schedule == 'multi-dlas-gpu': 
        if FLAGS01.scheme != 'count':
            ...
            exit()
    ''' Parse input'''
    parse_job_file(FLAGS01.trace_file)
    parse_cluster_spec()
    ...
    elif FLAGS01.schedule == 'lpjf':
        longest_pending_first_sim_jobs()
    elif FLAGS01.schedule == 'shortest':
        shortest_first_sim_jobs()
    elif FLAGS01.schedule == 'shortest-expected':
        JOBS01.job_dist_data = parse_job_dist()
        shortest_first_sim_jobs()
    elif FLAGS01.schedule == 'shortest-gpu':
        shortest_first_sim_jobs(True)
    elif FLAGS01.schedule == 'dlas':
        JOBS01.job_dist_data = parse_job_dist()
        dlas_sim_jobs()
    elif FLAGS01.schedule == 'dlas-gpu':
        dlas_sim_jobs(True)
    elif FLAGS01.schedule == 'dlas-gpu-gittins':
        JOBS01.job_dist_data = parse_job_dist()
        dlas_sim_jobs(True)
    elif FLAGS01.schedule == 'dlas-gpu-gittins-1':
        JOBS01.job_dist_data = parse_job_dist()
        dlas_sim_jobs(True, 1)
    elif FLAGS01.schedule == 'dlas-gpu-gittins-2':
        JOBS01.job_dist_data = parse_job_dist()
        dlas_sim_jobs(True, 2)
    elif FLAGS01.schedule == 'dlas-gpu-gittins-4':
        JOBS01.job_dist_data = parse_job_dist()
        dlas_sim_jobs(True, 4)
    elif FLAGS01.schedule == 'dlas-gpu-gittins-8':
        JOBS01.job_dist_data = parse_job_dist()
        dlas_sim_jobs(True, 8)
    elif FLAGS01.schedule == 'dlas-gpu-pack':
        CLUSTER01.init_dlas_pack_gpu()
        dlas_pack_sim_jobs(True)
    elif FLAGS01.schedule == 'multi-dlas-gpu':
        # JOBS.init_reserve_gpus(CLUSTER.num_gpu)
        # JOBS.test_reserve_gpus(CLUSTER.num_gpu)
        multi_dlas_sim_jobs(True)
    elif FLAGS01.schedule == 'gittins':
        # JOBS.init_reserve_gpus(CLUSTER.num_gpu)
        # JOBS.test_reserve_gpus(CLUSTER.num_gpu)
        job_dist_data = parse_job_dist()
        gittins_sim_jobs(job_dist_data, True, True)
    elif FLAGS01.schedule == 'dlas-gpu-1':
        dlas_sim_jobs(True,1)
    elif FLAGS01.schedule == 'dlas-gpu-2':
        dlas_sim_jobs(True,2)
    elif FLAGS01.schedule == 'dlas-gpu-05':
        dlas_sim_jobs(True, 0.5)
    elif FLAGS01.schedule == 'dlas-gpu-4':
        dlas_sim_jobs(True, 4)
    elif FLAGS01.schedule == 'dlas-gpu-8':
        dlas_sim_jobs(True, 8)
    elif FLAGS01.schedule == 'dlas-gpu-10':
        dlas_sim_jobs(True, 10)
    elif FLAGS01.schedule == 'dlas-gpu-100':
        dlas_sim_jobs(True, 100)
    elif FLAGS01.schedule == 'dlas-gpu-1000':
        dlas_sim_jobs(True, 1000)
    elif FLAGS01.schedule == 'gandiva':
        CLUSTER01.init_gandiva_nodes()
        gandiva_sim_jobs(True, 1000)
    elif FLAGS01.schedule == 'gpu-demands':
        sim_gpu_demands()
    else:
        one_queue_fifo_sim_jobs()
