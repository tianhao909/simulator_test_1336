...
#         self.pending_jobs = list() #等待的  # [{job_dict}, {job_dict}]
#         self.runnable_jobs = list() #可运行的 # pending + running
...
#             job_dict['model'] = models01.get_model_with_scale(job_dict['model_name'], job_dict['model_scale'])
#         else:
...
#             #for job start
#             tmp_dict = util01.search_dict_list(self.job_events, 'time', start_t)
#             if tmp_dict == None:
...
#         #util01.print_fn('Init, add job start events')
...
#         # self.read_all_jobs()      
#         if FLAGS01.schedule == 'multi-dlas-gpu' and FLAGS01.scheme == 'count':
...
#             del queue[:]  #删除列表list中所有的元素  del 用于删除对象。在 Python，一切都是对象，因此 del 关键字可用于删除变量、列表或列表片段等。
#         for job in self.runnable_jobs:
#             if gputime:
...
#         #for job end 
#         tmp_dict = util01.search_dict_list(self.job_events, 'time', job['end_time'])
#         if tmp_dict == None:
#             #not found, add the time into to job_events
#             tmp_dict = dict()
...
class _TFJobs(object):
    ...
    def sort_all_jobs(self, mode=None):
        ...
        util01.print_fn('   Jobs are sorted with their start time')
        # self.read_all_jobs()
        if FLAGS01.schedule == 'multi-dlas-gpu' and FLAGS.scheme == 'count':
            for num_gpu, gjob in self.gpu_job.items():
                util01.print_fn('%d-GPU jobs have %d ' % (num_gpu, gjob.total_job))
    ...
    def completion_check(self):
        for num_gpu, gjob in self.gpu_job.items():
            if gjob.end_job != gjob.total_job:
                util01.print_fn('!!!! Miss-match %d completed jobs with %d total jobs in %d-GPU jobs' % (gjob.end_job, gjob.total_job, num_gpu))
