...
from switch01 import _Switch
from node01 import _Node
...
#         #self.mem = num_switch * num_node_p_switch * mem_p_node
...
#             self.set_spec(FLAGS01.num_switch, FLAGS01.num_node_p_switch, FLAGS01.num_gpu_p_node, FLAGS01.num_cpu_p_node, FLAGS01.mem_p_node)
#         else:
...
#         '''
#         for switch01 in self.switch_list:
#             ret = switch01.ms_yarn_alloc_res(job)
#             if ret == True:
#                 return True
#             else:
#                 continue
...
class _Cluster(object):
    ...
    def init_gandiva_nodes(self):
        ...
        if len(self.free_nodes) != self.num_node:
            util01.print_fn("free_node: %s, is incorrect" % len(self.free_nodes))
            exit()
    ...
    def gandiva_placement(self, job):
        ...
        if num_gpu not in self.node_g:
            util01.print_fn("error: job[%d] needs %d GPUs" % (job['job_idx'], num_gpu))
            exit()
        ...
        if len(node_list) > 0:
            ...
            if round(ns_util + job_util, 2) < node_set['capacity']:
                ...
                util01.print_fn("job[%d] starts1" % job['job_idx'])
                return True
            else:
                node_list.append(node_set)
        ...
        if num_node <= len(self.free_nodes):
            ...
            util01.print_fn("job[%d] starts2" % job['job_idx'])
            return True
        ...
        if len(node_list) > 0:
            ...
            node_list.append(node_set)
            node_list.sort(key = lambda e:e.__getitem__('util'))
            util01.print_fn("job[%d] starts3" % job['job_idx'])
            return True
        else: #no existing node_set
            util01.print_fn("job[%d] pend 4, free nodes %d" % (job['job_idx'], len(self.free_nodes)))
            return False
    ...
    def time_slicing_execute(self, cur_time, jobs, logs, time_diff):
        ...
        used_gpus = 0
        util01.print_fn("free:%d, 1:%d, 2: %d, 4:%d, 8: %d, 16:%d, 32:%d, 64:%d\n" % (len(self.free_nodes), 
        ...
        for num_gpu, node_list in self.node_g.items():
            ...
            for node_set in node_list:
                ...
                util01.print_fn("%d-GPU con:%d, jobs:%d" % (num_gpu, concurrency, len(node_set['jobs'])))
                i = 0
                end_list = list()
                for r_job in node_set['jobs']:
